...
double add_num(double x, double y)
{
    ...
    if (isnan(x) || isnan(y))
    {
        printf("Invalid input: one of the inputs is NaN.\n");
        return -1; // Indicate an error
    }

    // Check for near-infinity values
    if (isinf(x) || isinf(y))
    {
        printf("Invalid input: one of the inputs is near infinity.\n");
        return -1; // Indicate an error
    }

    // Check for overflow
    if ((x > 0 && y > 0 && x > DBL_MAX - y) || (x < 0 && y < 0 && x < -DBL_MAX - y))
    {
        printf("Overflow error: result exceeds double value range.\n");
        return -1; // Indicate an error
    }
...
double multiply_num(double x, double y)
{
    ...
    // Check for overflow
    if ((x > 0 && y > 0 && x > DBL_MAX / y) || (x < 0 && y < 0 && x < -DBL_MAX / y) ||
        (x > 0 && y < 0 && x > -DBL_MAX / y) || (x < 0 && y > 0 && x < DBL_MAX / y))
    {
        printf("Overflow error: result exceeds double value range.\n");
        return -1; // Indicate an error
    }
